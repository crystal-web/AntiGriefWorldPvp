...
public class AntiGriefWorldPvp extends JavaPlugin implements Listener
{
		...
		if(label.equalsIgnoreCase("agpvp") && args.length > 0){
			if(args[0].equalsIgnoreCase("addblock")){
				...
				for(int i =2;i<args.length;i++){
					...
					player.sendMessage(ChatColor.GREEN + "Le bloque a bien été rajouté a la liste");
				}
			}
			if(args[0].equalsIgnoreCase("delblock")){
				...
				for(int i =2;i<args.length;i++){
					...
					player.sendMessage(ChatColor.GREEN + "Le bloque a bien été supprimé de la liste");
				}
			}
	...
	public void showHelp(Player player){
		...
		player.sendMessage("/agpvp addblock <world> <blockID>     - Ajoute un bloque à la blacklist");
		...
		player.sendMessage("/agpvp getmessage <world>     -Affiche le message lors d'un bloque blacklisté cassé");
